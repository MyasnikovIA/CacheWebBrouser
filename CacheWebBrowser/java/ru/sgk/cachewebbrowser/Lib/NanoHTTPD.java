package ru.sgk.cachewebbrowser.Lib;
...
import ru.sgk.cachewebbrowser.http.LocalHttpService;
...
public class NanoHTTPD
{
	...
	public Response serve( String uri, String method, Properties header, Properties parms, Properties files )
	{
		...
		return serveFile( uri, header, am, true );
	}
	 ...
	 */
	public NanoHTTPD( int port, AssetManager am ) throws IOException
	{
		myTcpPort = port;
		this.am = am;
	 ...
	 */
	public Response serveFile( String uri, Properties header, AssetManager am,
							   boolean allowDirectoryListing )
	{
		...
		try {
			...
			if(!uri.equals("cam.jpg")) {
				...
				YuvImage im = LocalHttpService.getImage();
				im.compressToJpeg(
		                new Rect(0, 0, im.getWidth(), im.getHeight()), 90, out);
				res = new Response( HTTP_OK, mime, new ByteArrayInputStream(out.toByteArray()));
			
				res.addHeader( "Content-Length", "" + out.size());
			}
		} catch (IOException e) {
